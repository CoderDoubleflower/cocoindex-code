...
def _user_settings_to_dict(settings: UserSettings) -> dict[str, Any]:
    d: dict[str, Any] = {}
    emb: dict[str, Any] = {}
    if settings.embedding.provider != "litellm":
        emb["provider"] = settings.embedding.provider
    if settings.embedding.model != "text-embedding-3-small":
        emb["model"] = settings.embedding.model
    if settings.embedding.device is not None:
        emb["device"] = settings.embedding.device
    if settings.embedding.api_base is not None:
        emb["api_base"] = settings.embedding.api_base
    if settings.embedding.encoding_format != "float":
        emb["encoding_format"] = settings.embedding.encoding_format
    if emb:
        d["embedding"] = emb
    if settings.envs:
...
def _user_settings_from_dict(d: dict[str, Any]) -> UserSettings:
    ...
    embedding = EmbeddingSettings(
        provider=emb_dict.get("provider", "litellm"),
        model=emb_dict.get("model", "text-embedding-3-small"),
        device=emb_dict.get("device"),
        api_base=emb_dict.get("api_base"),
        encoding_format=emb_dict.get("encoding_format", "float"),
    )
    envs = d.get("envs", {})
