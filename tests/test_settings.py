...
def test_default_user_settings() -> None:
    ...
    assert s.embedding.provider == "litellm"
    assert s.embedding.model == "text-embedding-3-small"
    assert s.embedding.device is None
    assert s.embedding.api_base is None
    assert s.embedding.encoding_format == "float"
    assert s.envs == {}
...
def test_save_and_load_user_settings(tmp_path: Path) -> None:
    ...
    assert loaded.embedding.api_base == settings.embedding.api_base
    assert loaded.embedding.encoding_format == settings.embedding.encoding_format
